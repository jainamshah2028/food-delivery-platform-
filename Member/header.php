<?php
session_start();
require_once(__DIR__ . '/../sidebar_template.php');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
:root{
  ...
  --glow:0 0 24px rgba(245,166,35,0.2);--shadow-lg:0 12px 40px rgba(0,0,0,0.6);
}
...
.user-avatar{width:34px;height:34px;background:linear-gradient(135deg,var(--accent),var(--accent2));border-radius:8px;display:flex;align-items:center;justify-content:center;font-family:'Segoe UI',system-ui,sans-serif;font-size:13px;font-weight:800;color:#000;flex-shrink:0;}
...
.btn-accent:hover{background:var(--accent3);box-shadow:var(--glow);}
...
.hero{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius-xl);padding:32px 36px;margin-bottom:24px;position:relative;overflow:hidden;}
.hero::after{content:'';position:absolute;inset:0;background-image:linear-gradient(rgba(255,255,255,0.015) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.015) 1px,transparent 1px);background-size:32px 32px;pointer-events:none;}
.hero-glow{position:absolute;top:-60px;right:-60px;width:280px;height:280px;background:radial-gradient(circle,rgba(245,166,35,0.1) 0%,transparent 70%);border-radius:50%;pointer-events:none;}
.hero-inner{position:relative;z-index:1;}
.hero-tag{display:inline-block;background:var(--accent-bg2);border:1px solid rgba(245,166,35,0.25);color:var(--accent);font-size:10px;font-weight:500;letter-spacing:2px;text-transform:uppercase;padding:5px 12px;border-radius:20px;margin-bottom:14px;}
.hero-title{font-family:'Segoe UI',system-ui,sans-serif;font-size:clamp(26px,3vw,36px);font-weight:800;color:var(--t1);line-height:1.1;margin-bottom:10px;letter-spacing:-0.5px;}
.hero-title em{color:var(--accent);font-style:normal;}
.hero-sub{font-size:14px;color:var(--t2);line-height:1.6;max-width:400px;}
.hero-deco{position:absolute;right:40px;top:50%;transform:translateY(-50%);font-size:72px;opacity:0.12;pointer-events:none;}
...
.cat-card:hover{border-color:var(--accent);background:var(--accent-bg);transform:translateY(-3px);box-shadow:0 8px 28px rgba(245,166,35,0.1);}
.cat-emoji{font-size:32px;margin-bottom:10px;display:block;transition:transform 0.2s;}
...
.cat-hint{font-size:11px;color:var(--t3);}
...
.prod-price{font-size:20px;font-weight:700;color:var(--accent);font-family:'Segoe UI',system-ui,sans-serif;}
.prod-desc{font-size:12px;color:var(--t3);margin:4px 0 12px;line-height:1.5;}
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:14px;margin-bottom:24px;}
.stat-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px 22px;display:flex;align-items:center;gap:14px;}
.stat-icon{width:42px;height:42px;border-radius:10px;background:var(--accent-bg);border:1px solid rgba(245,166,35,0.2);display:flex;align-items:center;justify-content:center;font-size:19px;flex-shrink:0;}
.stat-val{font-family:'Segoe UI',system-ui,sans-serif;font-size:24px;font-weight:800;color:var(--t1);line-height:1;}
...
th{font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:var(--t3);font-weight:500;padding:11px 14px;text-align:left;}
td{padding:13px 14px;font-size:13px;color:var(--t2);border-bottom:1px solid var(--border);}
...
.form-input:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(245,166,35,0.1);}
...
<aside class="sidebar">
  <div class="sidebar-logo">
    <div class="logo-mark">
      ...
      <div class="logo-text">Anand <span>FC</span></div>
    </div>
    <div class="logo-sub">Food Court</div>
  </div>
  <div class="sidebar-user">
    <div class="user-avatar"><?php echo strtoupper(substr($_SESSION["name"]??"U",0,1)); ?></div>
    <div>
  ...
  <nav class="nav-section">
    ...
    <a href="category.php" class="nav-item"><span class="nav-icon">🍽️</span> Order Food</a>
    <a href="seller.php" class="nav-item"><span class="nav-icon">🔥</span> Best Sellers</a>
    <a href="rated.php" class="nav-item"><span class="nav-icon">⭐</span> Top Rated</a>
    <span class="nav-label">Account</span>
    <a href="myorder.php" class="nav-item"><span class="nav-icon">📦</span> My Orders</a>
    <a href="myrecharge.php" class="nav-item"><span class="nav-icon">💳</span> Recharges</a>
    <a href="membership.php" class="nav-item"><span class="nav-icon">👑</span> Membership</a>
    <a href="myprofile.php" class="nav-item"><span class="nav-icon">👤</span> Profile</a>
    <a href="feedback.php" class="nav-item"><span class="nav-icon">💬</span> Feedback</a>
  </nav>
  <div class="sidebar-bottom">
    <a href="/anand_secured/login.php" class="logout-link"><span>🚪</span> Logout</a>
  </div>
</aside>

<div class="main">
<div class="topbar">
  <div class="topbar-title">Anand Food Court</div>
  <div class="topbar-right">
    <span class="topbar-greeting">Hey, <strong><?php echo htmlspecialchars($_SESSION["name"]??"Guest"); ?></strong> 👋</span>
    <a href="temp.php" class="btn btn-ghost">🛒 Cart</a>
    <a href="category.php" class="btn btn-accent">+ Order</a>
  </div>
</div>
